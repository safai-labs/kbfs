...
func TestTLFCryptKeyInfoV2EPubKeyIndex(t *testing.T) {
	infoV2 := TLFCryptKeyInfoV2{
	...
	expectedBuf := []byte{
		0x83, 0xaa, 0x43, 0x6c, 0x69, 0x65,
		0x6e, 0x74, 0x48, 0x61, 0x6c, 0x66, 0x83, 0xa1, 0x65, 0xc0,
		0xa1, 0x6e, 0xc0, 0xa1, 0x76, 0x0, 0xac, 0x53, 0x65, 0x72,
		0x76, 0x65, 0x72, 0x48, 0x61, 0x6c, 0x66, 0x49, 0x44, 0x81,
		0xa2, 0x49, 0x44, 0xc0, 0xa1, 0x69, 0xcc, 0x80,
	}

	codec := kbfscodec.NewMsgpack()
	buf, err := codec.Encode(infoV2)
	require.NoError(t, err)
	require.Equal(t, expectedBuf, buf)

	infoV2Neg := TLFCryptKeyInfoV2{
		// This should msgpack-encode as a int8 (0xd0).
		EPubKeyIndex: -33,
	}
	...
	expectedBufNeg := []byte{
		...
		0xa1, 0x6e, 0xc0, 0xa1, 0x76, 0x0, 0xac, 0x53, 0x65, 0x72,
		0x76, 0x65, 0x72, 0x48, 0x61, 0x6c, 0x66, 0x49, 0x44, 0x81,
		0xa2, 0x49, 0x44, 0xc0, 0xa1, 0x69, 0xd0, 0xdf,
	}

	bufNeg, err := codec.Encode(infoV2Neg)
	require.NoError(t, err)
	require.Equal(t, expectedBufNeg, bufNeg)
}
